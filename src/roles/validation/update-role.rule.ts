  ...
  implements ValidatorConstraintInterface
{
  ...
  async validate(id: string, args: ValidationArguments): Promise<boolean> {
    ...
    if (
      role.permissions.includes(Permissions.OWNER) ||
      role.permissions.includes(Permissions.ADMIN)
    )
      throw new BadRequestException('you are not update or add this role');
    return true;
  }
